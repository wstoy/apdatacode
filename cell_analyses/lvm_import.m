...
%
%
% This function imports a text-formatted LabView Measurement File
%  (LVM, extension ".lvm") into METLAB. A LVM file can have multiple
%  Segments, so that multiple measurements can be combined in a single
%  file. The output variable DATA will be a structure with fields named
...
%  'data'. The column labels and units are stored as cell arrays that
...
while 1
    ...
    % How many data columns have we? (including X data)
    ...
    if ~strcmp(data.Decimal_Separator,'.')
        ...
        % create a format string for reading data as strings (+1 for Comments)
        fs = '%s'; for i=2:numdatacols+1, fs = [fs ' %s']; end                  %#ok<AGROW>
        % read data from file
        ...
        rawdata = textscan(fid,fs,'delimiter',text_delimiter);
        % save first row comment as The Comment for this segment
        % IK Addition to check for empty files
        try
            data.(fieldnm).Comment = rawdata{size(rawdata,2)}{1};
        catch ME
            fclose(fid);
    ...
    data.(fieldnm).data=rawdata{1}; % start with X, even though it may be empty
    for i=2:numdatacols
    ...
    if isfield(data.(fieldnm),'num_samples')
        if length(data.(fieldnm).data) ~= data.(fieldnm).num_samples(1)
            if verbose >= 1
                fprintf(1,' WARNING: Number of data points read in segment (%g) is not equal to\n',length(data.(fieldnm).data));
                fprintf(1,'          number of samples shown in the segment header (%g)\n\n',data.(fieldnm).num_samples(1));
            end
        end
    end
...
if verbose >= 1, fprintf(1,' Import complete. File has %s and %d Data Segments.\n\n',xColPlural,segnum); end
